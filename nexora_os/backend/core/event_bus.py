...
class EventBus:
    def __init__(self, history_size: int = 500, enable_async_delivery: bool = False, queue_size: int = 1000) -> None:
    ...
    def _ensure_async_loop(self) -> None:
        ...
        if self._enable_async_delivery and self._event_queue is None:
            try:
                ...
                self._event_queue = asyncio.PriorityQueue(maxsize=self._queue_size)
    ...
    async def _delivery_loop(self) -> None:
        ...
        while True:
            try:
                _priority, _sequence, event = await self._event_queue.get()
                with self._lock:
        ...
        if self._enable_async_delivery and self._event_queue is not None:
            ...
            try:
                if self._loop and self._loop.is_running():
                    self._loop.call_soon_threadsafe(self._enqueue, self._event_queue, (priority_value, event.sequence, event))
                else:
                    self._enqueue(self._event_queue, (priority_value, event.sequence, event))
            except RuntimeError:
                with self._lock:
                    self._dropped += 1
        else:
    ...
    def _enqueue(self, queue: asyncio.PriorityQueue, item: tuple) -> None:
        # Scheduled callbacks must catch overflow where put_nowait executes.
        if queue is not self._event_queue:
            with self._lock:
                self._dropped += 1
            return
        try:
